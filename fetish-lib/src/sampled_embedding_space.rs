...
///All information in a [`SampledEmbedderState`] pertaining to a particular
///function [`TypeId`]
pub struct SampledEmbeddingSpace<'a> {
    pub type_id : TypeId,
    ///A sample drawn from the [`Elaborator`] for this type in the original [`EmbedderState`].
...
impl<'a> SampledEmbeddingSpace<'a> {
    ...
    ///first performs [`expand_compressed_vector`] and
