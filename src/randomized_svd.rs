...
fn randomized_range_finder(input : &Array2<f32>, size : usize, n_iter : usize, rng : &mut ThreadRng) -> 
    ...
    let omega = Array::random((input.ncols(), size), StandardNormal); 

    let mut y = input.dot(&omega);
    let mut q = y.qr().unwrap().0;
    for _ in 0..n_iter {
        let y_twiddle = input_t.dot(&q);
        let q_twiddle = y_twiddle.qr().unwrap().0;
        y = input.dot(&q_twiddle);
        q = y.qr().unwrap().0;
    }
    q
}
