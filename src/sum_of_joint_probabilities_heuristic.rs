use ndarray::*;
use ndarray_linalg::*;
use crate::sampled_embedding_space::*;
use crate::model_space::*;
use crate::model::*;
use crate::sampled_term_embedding::*;
use crate::schmear::*;
use crate::inverse_schmear::*;
use crate::pseudoinverse::*;
use crate::func_schmear::*;
use crate::ellipsoid::*;
use crate::chi_squared_inverse_cdf::*;
use crate::linear_sketch::*;
use crate::params::*;
use crate::bounded_hole::*;

pub fn sum_of_joint_probabilities_heuristic(embedding_space : &SampledEmbeddingSpace, bound : &BoundedHole) -> f32 {
    let dim = bound.compressed_bound.dims();
    let chi_squared_value = chi_squared_inverse_cdf_for_heuristic_ci(dim);

    let ellipsoid_center = bound.compressed_bound.center();
    let ellipsoid_skew = bound.compressed_bound.skew();
    let ellipsoid_skew_inv = pseudoinverse_h(ellipsoid_skew);
    //TODO: Check if this is indeed multiplication, or should it be division?
    let scaled_ellipsoid_skew_inv = chi_squared_value * ellipsoid_skew_inv;

    let ellipsoid_schmear = Schmear {
        mean : ellipsoid_center.clone(),
        covariance : scaled_ellipsoid_skew_inv
    };

    let mut tot = 0.0f32;
    for (model_key, embedding) in &embedding_space.models {
        let model_schmear = &embedding.compressed_schmear;
        let joint_probability_integral = ellipsoid_schmear.joint_probability_integral(model_schmear);
        tot += joint_probability_integral;
    }
    tot
}
